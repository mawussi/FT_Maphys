...
#define ASSRT( x ) Call mph_assert( (x) , __LINE__, FLNAME );
#define MPH_ONFAILURE_GOTO9999(x) Call mph_check((x>=0),__LINE__,x,FLNAME);If(x<0)Goto 9999
#define MPH_ONFAILURE_ABORT(x) Call mph_assert((x>=0), __LINE__, FLNAME );
#define MPH_ONFAILURE_RETURN(x) Call mph_check((x>=0),__LINE__,x,FLNAME);If(x<0)Return
#else
#define CHCKASSRT( x, res ) If(.NOT.(x)) res = - __LINE__ ;
#define ASSRT( x ) If(.NOT.(x)) Call mph_abort ;
#define MPH_ONFAILURE_GOTO9999(x) If(x<0)Then; x=__LINE__; Goto 9999; End If
#define MPH_ONFAILURE_ABORT(x) If(x<0) Call mph_abort ;
