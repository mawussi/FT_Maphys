 ...
 * @return "a-b"
 */
int mph_qsort_comp(const void * a,const void * b)
{
  return ( (*(int *)a) - (*(int *)b) );
}
 ...
 * 
*/
void mph_qsortc(int *arrayAndperm, int* size)
{
  qsort(arrayAndperm,(*size), 2*sizeof(int), mph_qsort_comp) ;
}

/* Fortran binding to maphys_qsortc */
void MPH_QSORTC(int *arrayAndperm, int* size)
{
  mph_qsortc(arrayAndperm,size);
}

void mph_qsortc_(int *arrayAndperm, int* size)
{
  mph_qsortc(arrayAndperm,size);
}

void mph_qsortc__(int *arrayAndperm, int* size)
{
  mph_qsortc(arrayAndperm,size);
}
