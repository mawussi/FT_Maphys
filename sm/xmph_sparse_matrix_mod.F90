...
!> MAPHYS sparse matrix module.
!!
!! set of routines to create and manipulate sparse matrices
!!
Module XMPH_sparse_matrix_mod

  !* Module(s) *!
  Use mph_error_mod
  Use XMPH_sparse_matrix_type ! specify the structure and its related enumerations 
  ...
  Character(len=MAPHYS_STRL), Private, Parameter :: FLNAME = &
       "XMPH_ARITHmph_sparse_matrix_mod.F90"
  ...
  Public :: XMPH_iset
  Public :: XMPH_xset
  Public :: XMPH_irealloc
  Public :: XMPH_iperm
  Public :: XMPH_xperm
  Public :: XMPH_invperm
  Public :: XMPH_qsort
  Public :: XMPH_sm_ind2ptr
  Public :: XMPH_sm_ptr2ind


  ! creation / deletion
  Public :: XMPH_sm_nullify
  Public :: XMPH_sm_free
  Public :: XMPH_sm_ijv
  Public :: XMPH_sm_create
  Public :: XMPH_sm_createFromData
  Public :: XMPH_sm_dup
  Public :: XMPH_sm_insertEntry 
  Public :: XMPH_sm_realloc

 
  ! IO
  Public :: XMPH_sm_mmread
  Public :: XMPH_sm_ijvread
  Private :: XMPH_sm_ijvReadData
  Public :: XMPH_sm_hbread
  Public :: XMPH_sm_mmwrite

  ! others 
  Public :: XMPH_sm_sizeof
  Public :: XMPH_sm_check
  Public :: XMPH_sm_mirror
  Public :: XMPH_sm_convert
  Public :: XMPH_sm_get_submatrices   



  ! mathematic operations
  Public :: XMPH_sm_transposeUpper
  Public :: XMPH_sm_assemble
  Public :: XMPH_sm_symStruct
  Private :: HasEntry
  Public :: XMPH_sm_vectorproduct 

  ! MPI operations
  Public :: XMPH_sm_bcast
  Public :: XMPH_sm_allgather
...
Contains
  ...
  !!                      
  Subroutine XMPH_icopy(array, tocopy , size) 

    !* Arguments *!

    MPH_INT , pointer , intent(inout) :: array  (:)
    MPH_INT , pointer , intent(in   ) :: tocopy (:)
    MPH_INT           , intent(in   ) :: size
  ...
  End Subroutine XMPH_icopy


  ! [+] routine : XMPH_xcopy -------------------------------------------------
  !
  !> copy an XMPH_FLOAT array
  !!
  !! @param[in,out] array    the array             (size)
  !! @param[in    ] tocopy   the array to copy     (size)
  !! @param[in    ] size   the size of the array
  !!                      
  Subroutine XMPH_xcopy(array, tocopy , size) 

    !* Arguments *!

    XMPH_FLOAT , pointer , intent(inout) :: array  (:)
    XMPH_FLOAT , pointer , intent(in   ) :: tocopy (:)
    MPH_INT           , intent(in   ) :: size
  ...
  End Subroutine XMPH_xcopy

  ! [+] routine : XMPH_idup -------------------------------------------------
  !
  !> duplicate an MPH_INT integer array
  !!
  !! @param[in,out] array    the array             (size)
  !! @param[in    ] tocopy   the array to copy     (size)
  !! @param[in    ] size     the size of the array
  ...
  !!                      
  Subroutine XMPH_idup(array, tocopy , size, info) 

    !* Arguments *!

    MPH_INT , Pointer , Intent(  out) :: array  (:)
    MPH_INT , Pointer , Intent(in   ) :: tocopy (:)
    MPH_INT           , Intent(in   ) :: size
    ...
    Call XMPH_icopy(array,tocopy,size)

  End Subroutine XMPH_idup

  ! [+] routine : XMPH_xdup -------------------------------------------------
  !
  !> duplicate an XMPH_FLOAT integer array
  !!
  !! @param[in,out] array    the array             (size)
  !! @param[in    ] tocopy   the array to copy     (size)
  !! @param[in    ] size     the size of the array
  !! @param[   out] info     the return status
  !!                      
  Subroutine XMPH_xdup(array, tocopy , size, info) 

    !* Arguments *!

    XMPH_FLOAT , Pointer , Intent(  out) :: array  (:)
    XMPH_FLOAT , Pointer , Intent(in   ) :: tocopy  (:)
    MPH_INT           , Intent(in   ) :: size
    Integer           , Intent(  out) :: info
    ...
    Call XMPH_xcopy(array,tocopy,size)

  End Subroutine XMPH_xdup


  ! [+] routine : XMPH_iset -------------------------------------------------
  !
  !> set a MPH_INT array to a value
  !!
  ...
  !!                      
  Subroutine XMPH_iset(i, istart , iend, ival ) 

    !* Arguments *!

    MPH_INT , Pointer , Intent(inout) :: i     (:)
    MPH_INT           , Intent(in   ) :: istart
    MPH_INT           , Intent(in   ) :: iend
    MPH_INT           , Intent(in   ) :: ival
  ...
  End Subroutine XMPH_iset

  ! [+] routine : XMPH_xset -------------------------------------------------
  !
  !> set a XMPH_FLOAT array to a value
  !!
  ...
  !!                      
  Subroutine XMPH_xset(x, xstart , xend, xval ) 

    !* Arguments *!

    XMPH_FLOAT , Pointer , Intent(inout) :: x     (:)
    MPH_INT           , Intent(in   ) :: xstart
    MPH_INT           , Intent(in   ) :: xend
    XMPH_FLOAT        , Intent(in   ) :: xval
  ...
  End Subroutine XMPH_xset


  ! [+] routine : XMPH_irealloc -------------------------------------------------
  !
  !> reallocate a MPH_INT array to a new size
  !!
  !! @param[in,out] array     the array             (size)
  ...
  !!                      
  Subroutine XMPH_irealloc( array , newsize, oldsize, info ) 

    !* Arguments *!

    MPH_INT , Pointer , Intent(inout) :: array (:)
    MPH_INT           , Intent(in   ) :: newsize
    MPH_INT           , Intent(in   ) :: oldsize
    Integer           , Intent(  out) :: info
    ...
    Call XMPH_icopy(tmp,array,Min(oldsize,newsize))
    ...
    If ( newsize > oldsize )Then
       Call XMPH_iset(tmp,oldsize+1,newsize,0)
    End If
  ...
  !!                      
  Subroutine XMPH_iperm(array, perm, size, istat) 
  ...
  !! @param[   out] istat  the routine status
  !!
  Subroutine XMPH_xperm(array, perm, size, istat) 
  ...
  End Subroutine XMPH_xperm

  ! [+] routine : XMPH_invperm ---------------------------------------------èèèè
  !
  ...
  !! @param[   out] istat  the routine status 
  !!                      
  Subroutine XMPH_invperm(array, perm, size, istat) 
  ...
  !! 
  Subroutine XMPH_qsort(size, array, perm, info )
    ...
    MPH_INT           ,Intent(in   ) :: size
    MPH_INT  ,Pointer ,Intent(inout) :: array (:)
    MPH_INT  ,Pointer ,Intent(inout) :: perm (:)
    Integer              ,Intent(  out) :: info
    ...
    Call MPH_qsortc( arrayAndperm(1), size )
  ...
  End Subroutine XMPH_qsort


  ! [+] routine : XMPH_sm_ind2ptr ----------------------------------------
  !
  ...
  !! @author Azzam Haidar
  !! @author Yohan Lee-tin-yien
  Subroutine XMPH_sm_ind2ptr( indsize, ind, ptrsize, ptr )

    !* Arguments *!

    MPH_INT, Intent(in ) :: indsize
    MPH_INT, Intent(in ) :: ind(:)
    MPH_INT, Intent(in ) :: ptrsize
    MPH_INT, Intent(out) :: ptr(:)
  ...
  End Subroutine XMPH_sm_ind2ptr

  ! [+] routine : XMPH_sm_ptr2ind ----------------------------------------
  !
  ...
  !! @author Yohan Lee-tin-yien
  !!
  Subroutine XMPH_sm_ptr2ind(ptrsize,ptr,ind)

    !* Arguments *!

    MPH_INT, Intent(in ) :: ptrsize
    MPH_INT, Intent(in ) :: ptr(:)
    MPH_INT, Intent(out) :: ind(:)
  ...
  End Subroutine XMPH_sm_ptr2ind


  ! [+] routine : XMPH_sm_nullify ----------------------------------------
  !
  ...
  !! @author Yohan Lee-tin-yien
  !!
  Subroutine XMPH_sm_nullify(sm, istat)
  ...
  End Subroutine XMPH_sm_nullify


  ! [+] routine : XMPH_sm_free -------------------------------------------
  !
  ...
  !! @author Yohan Lee-tin-yien
  !!
  Subroutine XMPH_sm_free(sm,info )

    !* Arguments *!

    Type(XMPH_sparse_matrix_t), intent(inout) :: sm
    Integer              , intent(  out) :: info 
  ...
  Subroutine XMPH_sm_ijv &
    ...
    XMPH_FLOAT, Intent(in), Target ::  v(:)   !< values
    ...
    Return
  End Subroutine XMPH_sm_ijv


  ! [+] routine : XMPH_sm_create -----------------------------------------
  ...
  !! @author Yohan Lee-tin-yien
  !!
  Subroutine XMPH_sm_create(sm,nnz,info)

    !* Arguments *! 
    Type(XMPH_sparse_matrix_t), Intent(inout) :: sm
    MPH_INT           , Intent(in   ) :: nnz
    Integer              , Intent(  out) :: info
  ...
  End Subroutine XMPH_sm_create

  ! [+] routine : XMPH_sm_createFromData -----------------------------
  ...
  !! @author Yohan Lee-tin-yien
  !!
  Subroutine XMPH_sm_createFromData(sm,fmt,sym,&
       m,n,nnz, i,j,v, info)
    ...
    MPH_INT             , Intent(in) :: n        
    MPH_INT             , Intent(in) :: nnz      
    MPH_INT, pointer    , Intent(in) :: i   (:)  
    MPH_INT, pointer    , Intent(in) :: j   (:)  
    XMPH_FLOAT, pointer , Intent(in) :: v   (:)  
    Integer              , Intent(  out) :: info
    ...
    Select Case (fmt)
    Case( SM_FMT_IJV ) 
       Call XMPH_icopy(sm%i,i, sm%nnz)
       Call XMPH_icopy(sm%j,j, sm%nnz)
       Call XMPH_xcopy(sm%v,v, sm%nnz)
    Case( SM_FMT_CSR )
       Call XMPH_icopy(sm%csr,i,m+1)
       Call XMPH_sm_ptr2ind(m+1,sm%csr,sm%i)
       Call XMPH_icopy(sm%j,j, sm%nnz)
       Call XMPH_xcopy(sm%v,v, sm%nnz)
    Case( SM_FMT_CSC )
       Call XMPH_icopy(sm%i,i, sm%nnz)
       Call XMPH_icopy(sm%csc,j, n+1)
       Call XMPH_sm_ptr2ind(n+1,sm%csc,sm%j)
       Call XMPH_xcopy(sm%v,v, sm%nnz)
    End Select


  End Subroutine XMPH_sm_createFromData
  ...
  !! @param[   out] info   the return status 
  !!
  Subroutine XMPH_sm_dup(smout, smin, info )
    Implicit None

    !* Arguments *!

    Type(XMPH_sparse_matrix_t), Intent(in ) :: smin
    Type(XMPH_sparse_matrix_t), Intent(out) :: smout
    Integer                   , Intent(out) :: info 

    !- End of header----------------------------------------------------------

    Call XMPH_sm_nullify(smout,info)
    ...
    !
    Call XMPH_idup(smout%i,smin%i,smout%nnz,info)
    MPH_ONFAILURE_GOTO9999(info)

    Call XMPH_idup(smout%j,smin%j,smout%nnz,info)
    MPH_ONFAILURE_GOTO9999(info)

    Call XMPH_xdup(smout%v,smin%v,smout%nnz,info)
    MPH_ONFAILURE_GOTO9999(info)

    ! 
    If ( smout%fmt == SM_FMT_CSR ) Then ! CSR
       Call XMPH_idup(smout%csr,smin%csr,smout%m+1,info)
       MPH_ONFAILURE_GOTO9999(info)
    End If

    If ( smout%fmt == SM_FMT_CSC ) Then ! CSC
       Call XMPH_idup(smout%csc,smin%csc,smout%n+1,info)
       MPH_ONFAILURE_GOTO9999(info)
    End If
...
9999 Continue
    ...
    If (info<0) Call XMPH_sm_free(smout,info)

  End Subroutine XMPH_sm_dup



  ! [+] routine : XMPH_sm_insertEntry ------------------------------------
  !
  ...
  !! @author Yohan Lee-tin-yien
  !!
  Subroutine XMPH_sm_insertEntry(sm,index, i,j,v, info)

    !* Arguments *!

    Type(XMPH_sparse_matrix_t), intent(inout) :: sm
    MPH_INT           , intent(in   ) :: index
    MPH_INT           , intent(in   ) :: i
    MPH_INT           , intent(in   ) :: j
    XMPH_FLOAT         , intent(in   ) :: v
    Integer              , intent(  out) :: info
  ...
  End Subroutine XMPH_sm_insertEntry



  ! [+] routine : XMPH_sm_realloc ----------------------------------------
  !
  ...
  !! @author Yohan Lee-tin-yien
  !!
  Subroutine XMPH_sm_realloc(sm, newnnz, info)
    ...
    ! arrays
    MPH_INT , Pointer       :: newi(:)
    MPH_INT , Pointer       :: newj(:)
    XMPH_FLOAT , Pointer       :: newv(:)
    ...
    Call XMPH_icopy(newi,sm%i,nnzcopied)
    Call XMPH_icopy(newj,sm%j,nnzcopied)
    Call XMPH_xcopy(newv,sm%v,nnzcopied)
    ...
    If ( newnnz > sm%nnz )Then
       Call XMPH_iset(newi,sm%nnz+1,newnnz,0)
       Call XMPH_iset(newj,sm%nnz+1,newnnz,0)
       Call XMPH_xset(newv,sm%nnz+1,newnnz,XMPH_FLOATZERO)
    End If
...
9999 Continue
  ...
  End Subroutine XMPH_sm_Realloc


  ! [+] routine : XMPH_sm_mmread -----------------------------------------
  !
  !> read a sparse matrix from matrix market file. 
  !!
  !! The matrix in the file must follow maphys float representation 
  ...
  !!  ...
  !! @endverbatim
  Subroutine XMPH_sm_mmread(sm, iu, info )
    Implicit None
    ...
    !* Local Variables *!
    Integer :: k
    Character(len=MAPHYS_STRL) ::  string
    ...
    Call XMPH_sm_ijvReadData(sm, iu, info )
    MPH_ONFAILURE_RETURN(info)
    
  End Subroutine XMPH_sm_mmread


  ! [+] routine : XMPH_sm_ijvread ----------------------------------------
  !
  ...
  !! @endverbatim
  !!
  Subroutine XMPH_sm_ijvread(sm, iu, info)
  ...
  End Subroutine XMPH_sm_ijvread

  ! [+] routine : XMPH_sm_ijvReadData ------------------------------------
  !
  ...
  !! @endverbatim
  !!
  Subroutine XMPH_sm_ijvReadData(sm, iu, info )
    ...
    Integer      :: k
#if ( XMPH_HAVE_ARITH_C ) || ( XMPH_HAVE_ARITH_Z )
  ...
  End Subroutine XMPH_sm_ijvReadData


  ! [+] routine : XMPH_sm_hbread -----------------------------------------
  !
  ...
  !!  
  Subroutine XMPH_sm_hbread(sm, iu, info )
    ...
    Call XMPH_sm_ptr2ind(sm%n+1, sm%csc, sm%j)
...
9999 Continue
    If (info < 0) Call XMPH_sm_free(sm,info)
    
  End Subroutine XMPH_sm_hbread


  ! [+] routine : XMPH_sm_mmwrite ----------------------------------------
  !
  ...
  !! @see XMPH_sm_mmread()
  !!
  Subroutine XMPH_sm_mmwrite(sm,funit,info)
    ...
    ! Constants
    Character(len=MAPHYS_STRL), Parameter :: MMbegin = &
         "%%MatrixMarket matrix coordinate"
    Character(len=MAPHYS_STRL), Parameter :: st_err = &
         "Error: in XMPH_sm_mmwrite, "

#if ( XMPH_HAVE_ARITH_S ) || ( XMPH_HAVE_ARITH_D )
    Character(len=MAPHYS_STRL), Parameter :: MMfmt   = '(2I10,1PE26.18)'
    Character(len=MAPHYS_STRL), Parameter :: MMarith = " real"
#elif ( XMPH_HAVE_ARITH_C ) || ( XMPH_HAVE_ARITH_Z )
    Character(len=MAPHYS_STRL), Parameter :: MMfmt   = "(2I10,1PE26.18,1PE26.18)"
    Character(len=MAPHYS_STRL), Parameter :: MMarith = " complex"
#endif
    ...
    ! Strings
    Character(len=MAPHYS_STRL) :: MMsym 
  ...
  End Subroutine XMPH_sm_mmwrite

  ! [+] function : XMPH_sm_sizeof --------------------------------------
  !
  ...
  !!
  function XMPH_sm_sizeof(sm) RESULT(mem)
    ...
    mem = 0
    mem = mem + sm%nnz*XMPH_FLOATBYTESIZE     ! v
    mem = mem + sm%nnz*MPH_INTBYTESIZE  *2 ! i,j

    If ( sm%fmt == SM_FMT_CSC ) mem = mem + (sm%n+1)*(MPH_INTBYTESIZE)
    If ( sm%fmt == SM_FMT_CSR ) mem = mem + (sm%m+1)*(MPH_INTBYTESIZE)
    
    mem = mem + 2*4                  ! flags (fmt,sym)
    mem = mem + 3*MPH_INTBYTESIZE ! sizes (m,n,nnz)

  End function XMPH_sm_sizeof

  ! [+] routine : XMPH_sm_check ------------------------------------------
  !
  ...
  End Subroutine XMPH_sm_check

  ! [+] routine : XMPH_sm_mirror -----------------------------------------
  !
  ...
  !! @endverbatim
  !!
  Subroutine XMPH_sm_mirror(sm,info)
    ...
    ! scalar
    MPH_INT :: i_order ! prefixes : i_ = input, o_ = output
    MPH_INT :: i_nnz   
    MPH_INT :: o_nnz
    MPH_INT :: i       ! dummy integer on inputs
    MPH_INT :: o       ! dummy integer on outputs

    ! arrays
    MPH_INT  , Pointer :: i_row (:)
    MPH_INT  , Pointer :: i_col (:)
    XMPH_FLOAT, Pointer :: i_val (:)

    MPH_INT  , Pointer :: o_row (:)
    MPH_INT  , Pointer :: o_col (:)
    XMPH_FLOAT, Pointer :: o_val (:)
    ...
    Call XMPH_icopy(o_row,i_row,i_nnz)
    Call XMPH_icopy(o_col,i_col,i_nnz)
    Call XMPH_xcopy(o_val,i_val,i_nnz)
  ...
  End subroutine XMPH_sm_mirror



  ! [+] routine : XMPH_sm_convert ----------------------------------------
  !
  ...
  !! @author Yohan Lee-tin-yien
  !!
  Subroutine XMPH_sm_convert( sm, fmt, info )
    ...
    csr2ijv: if ( sm%fmt == SM_FMT_CSR )then
       Call XMPH_sm_ptr2ind(sm%m+1,sm%csr,sm%i)
    ...
    ijv2csrc:if ( fmt /= SM_FMT_IJV )then
       ...
       Call XMPH_qsort(sm%nnz, master, perm, info)
       MPH_ONFAILURE_RETURN(info)

       Call XMPH_iperm(slave,perm,sm%nnz,info)
       MPH_ONFAILURE_RETURN(info)
       
       Call XMPH_xperm(sm%v  ,perm,sm%nnz,info)
       MPH_ONFAILURE_RETURN(info)
       ...
       Call XMPH_sm_ind2ptr(sm%nnz, master, cssize, cs)
  ...
  End Subroutine XMPH_sm_convert

  ! [+] routine : XMPH_sm_get_submatrices --------------------------------
  !
  ...
  !!
  !! @version 0.1
  Subroutine XMPH_sm_get_submatrices( & ! intents
       Aii_ndof, sm,                         & ! in
       Aii, Aib,                            & ! out
       Abi, Abb,                            &
       info                                 &
    ...
    Type(XMPH_sparse_matrix_t), Intent(in   ) :: sm
    Type(XMPH_sparse_matrix_t), Intent(  out) :: Aii
    Type(XMPH_sparse_matrix_t), Intent(  out) :: Aib
    Type(XMPH_sparse_matrix_t), Intent(  out) :: Abi
    Type(XMPH_sparse_matrix_t), Intent(  out) :: Abb
    Integer              , Intent(  out) :: info
    ...
    MPH_INT :: A_ndof   !< number of rows/columns in A
    MPH_INT :: Abb_ndof !< number of rows/columns in Abb 
    ...
    MPH_INT :: k,l,m,n,p !< dummy counters

    MPH_INT   :: i       !< the row    index of an entry
    MPH_INT   :: j       !< the column index of an entry
    XMPH_FLOAT :: v       !< the value        of an entry
    ...
    Call XMPH_sm_nullify(Aii,info_ignore)
    Call XMPH_sm_nullify(Abb,info_ignore)
    Call XMPH_sm_nullify(Abi,info_ignore)
    Call XMPH_sm_nullify(Aib,info_ignore)
    ...
    ! allocate memory
    Call XMPH_sm_create(Aii,Aii_nnz,info)
    MPH_ONFAILURE_GOTO9999(info)

    Call XMPH_sm_create(Abb,Abb_nnz,info)
    MPH_ONFAILURE_GOTO9999(info)

    Call XMPH_sm_create(Abi,Abi_nnz,info)
    MPH_ONFAILURE_GOTO9999(info)

    Call XMPH_sm_create(Aib,Aib_nnz,info)
    MPH_ONFAILURE_GOTO9999(info)
...
9999 Continue
    ...
    If (info < 0) Then
       Call XMPH_sm_free(Aii,info_ignore)
       Call XMPH_sm_free(Abb,info_ignore)
       Call XMPH_sm_free(Aib,info_ignore)
       Call XMPH_sm_free(Abi,info_ignore)
    End If

  End Subroutine XMPH_sm_get_submatrices



  ! [+] routine :  XMPH_SM_VectorProduct ---------------------------------
  !
  ...
  !!
  !!
  Subroutine XMPH_SM_VectorProduct &
    ...
    Type(XMPH_sparse_matrix_t), Intent(in   ) :: sm
    MPH_INT          , Intent(in   ) :: row_displ
    MPH_INT          , Intent(in   ) :: col_displ
    Type(XMPH_dense_matrix_t), Intent(in   ) :: x
    Type(XMPH_dense_matrix_t), Intent(inout) :: y
    Integer             , Intent(  out) :: info
    ...
    MPH_INT  , Pointer :: Mi (:)
    MPH_INT  , Pointer :: Mj (:)
    XMPH_FLOAT, Pointer :: Mv (:)
    XMPH_FLOAT, Pointer :: Xv (:)
    XMPH_FLOAT, Pointer :: Yv (:)
    ...
    Call XMPH_xset(Yv,1,n,XMPH_FLOATZERO)
  ...
  End Subroutine XMPH_SM_VectorProduct

  ! [+] routine : XMPH_sm_bcast ------------------------------------------
  !
  ...
  !! @author Yohan Lee-tin-yien
  !!
  Subroutine XMPH_sm_bcast(sm, root, comm, info  )
    ...
    Call MPI_Bcast(attributes, 5,MPH_INTMPI,root,comm,info)
    ASSRTMPI(info)
    ...
    If (rank /= root)Then
       ...
       Call XMPH_sm_Create(sm, attributes(3),info)
    ...
    Call MPI_Bcast (sm%i, sm%nnz, MPH_INTMPI  , root, comm, info)
    ASSRTMPI(info)

    Call MPI_Bcast (sm%j, sm%nnz, MPH_INTMPI  , root, comm, info)
    ASSRTMPI(info)
    
    Call MPI_Bcast (sm%v, sm%nnz, XMPH_FLOATMPI, root, comm, info)
    ASSRTMPI(info)

    !
    Select Case(sm%fmt)
    Case(SM_FMT_CSR)
       Call MPI_Bcast(sm%csr, sm%m+1, MPH_INTMPI,root, comm, info)
       ASSRTMPI(info)

    Case(SM_FMT_CSC)
       Call MPI_Bcast(sm%csc, sm%n+1, MPH_INTMPI,root, comm, info)
       ASSRTMPI(info)
  ...
  End Subroutine XMPH_sm_bcast

  ! [+] routine : XMPH_sm_Allgather --------------------------------------
  !
  ...
  Subroutine XMPH_sm_Allgather &
    ...
    include 'mpif.h'
    Type(XMPH_sparse_matrix_t), intent(in   ):: sm_send
    Type(XMPH_sparse_matrix_t), intent(inout):: sm_recv
    Integer              , intent(in   ):: comm
    MPH_INT           , intent(in   ):: global_n
    Integer              , intent(  out):: info
    ...
    Call XMPH_sm_create(sm_recv, sm_recv%nnz, info )
    MPH_ONFAILURE_ABORT(info)
    ...
    Call MPI_Allgatherv                            &
         (sm_send%v     , sm_send%nnz,XMPH_FLOATMPI, &
         sm_recv%v, nnz_all, displ, XMPH_FLOATMPI,   &
         comm, info)
    ASSRTMPI(info)
  ...
  End Subroutine XMPH_sm_Allgather

  ! [+] routine : XMPH_sm_transposeUpper ---------------------------------
  !
  !> Transpose the entries in the sparse matrix upper triangular part.
  !!
  !! @param[in,out ] M             the sparse matrix
  !!
  Subroutine XMPH_sm_transposeUpper(sm)
  ...
  End Subroutine XMPH_sm_transposeUpper

  ! [+] routine : XMPH_sm_assemble ---------------------------------------
  !
  ...
  !! @version 0.1
  !!
  Subroutine XMPH_sm_assemble( sm , global_n, info )
    ...
    ! arrays
    MPH_INT, Pointer :: perm (:), jptr(:)
    XMPH_FLOAT, Pointer :: vptr(:)
    ...
    !!
    !!  the XMPH_sm_convert subroutine have stability issue for
    !!  very large duppicated entries on very large problems.
    !!  This is due to IPSORT function stability issue.
    !!
    !!  A partial solution is to use HSL sort functions : kb07ai_mod.
    ...
    Call XMPH_sm_convert(sm, SM_FMT_CSR, info )
    MPH_ONFAILURE_GOTO9999(info)
    ...
    Do i=1,global_n
       ...
       Call XMPH_qsort(k, jptr, perm, info )
       MPH_ONFAILURE_GOTO9999(info)

       Call XMPH_xperm(vptr, perm, k, info)
       MPH_ONFAILURE_GOTO9999(info)
    ...
    ! update csr
    Call XMPH_sm_ind2ptr(sm%nnz, sm%i, sm%m+1, sm%csr )
...
9999 Continue
  ...
  End Subroutine XMPH_sm_assemble

  ! [+] routine : XMPH_sm_symStruct --------------------------------------
  !
  ...
  !! @author Yohan Lee-tin-yien
  !! 
  Subroutine XMPH_sm_symStruct(sm, info)
    ...
    ! type
    Type(XMPH_sparse_matrix_t) :: smadd
    ...
    ! init smadd
    smaddsize = INT(sm%n/4,kind=MPH_INTKIND)
    Call XMPH_sm_nullify(smadd, info )
    MPH_ONFAILURE_RETURN(info)
    Call XMPH_sm_create(smadd, smaddsize, info)
    MPH_ONFAILURE_RETURN(info)
    
    ! sort and append to a compressed vector if necessary
    If (sm%fmt == SM_FMT_IJV )Then
       Call XMPH_sm_convert(sm, SM_FMT_CSC, info)
       MPH_ONFAILURE_GOTO9999(info)
    End If
    ...
    Do k=1,sm%nnz
       ...
       If (cnt > smadd%nnz )Then ! smadd too small
          smaddsize = INT(smadd%nnz*1.5,KIND=MPH_INTKIND )+1
          Call XMPH_sm_realloc(smadd, smaddsize,info)
          MPH_ONFAILURE_GOTO9999(info)
       End If

       Call XMPH_sm_insertEntry(smadd,cnt,j,i,XMPH_FLOATZERO,info)
       MPH_ONFAILURE_GOTO9999(info)
    ...
    nnz = sm%nnz
    Call XMPH_sm_realloc(sm, nnz + cnt,info)
    MPH_ONFAILURE_GOTO9999(info)
    ...
    If ( cnt > 0 )Then ! appending data broke the csr/csc format.
       Call XMPH_sm_convert(sm,SM_FMT_IJV,info)
       MPH_ONFAILURE_GOTO9999(info)
    End If
...
9999 Continue
    Call XMPH_sm_free(smadd,info)


  End Subroutine XMPH_sm_symStruct
...
End Module XMPH_sparse_matrix_mod
